// Pre-flight scan for tasks-batch-15.sql
import fs from 'node:fs/promises'

const sql = await fs.readFile('tasks-batch-15.sql', 'utf-8')

// 1. Em-dash check
const emDashCount = (sql.match(/—/g) || []).length
console.log(`Em-dash count: ${emDashCount}`)

// 2. Contraction scan (don't, won't, can't, etc.)
...
for (let i = 0; i < faqBlocks.length; i++) {
  ...
  // Check for unescaped single quotes inside the SQL (should be doubled)
...
// 4. Quoted speech detection inside JSON values (S121 lesson)
// Looking for patterns like: said, 'word OR : 'word inside JSON content
